...
import { CanvasNodeSvgEditDialog } from "../components/canvas-node-svg-edit-dialog";
...
function InfiniteCanvasPage() {
    ...
    const deleteNodes = useCallback(
        (ids: Set<string>) => {
            ...
            setSvgEditNodeId((current) => (current && allIds.has(current) ? null : current));
    ...
    const downloadNodeImage = useCallback((node: CanvasNodeData) => {
        ...
        saveAs(node.metadata.content, `canvas-${node.type}-${node.id}.${node.type === CanvasNodeType.Video ? "mp4" : node.type === CanvasNodeType.Audio ? audioExtension(node.metadata.mimeType) : imageExtension(node.metadata.content, node.metadata.mimeType)}`);
    }, []);
    ...
    const openSvgEditor = useCallback(
        (node: CanvasNodeData) => {
            if (node.type !== CanvasNodeType.Image || !node.metadata?.content) {
                message.warning("图片节点为空，无法编辑 SVG");
                return;
            }
            if (!node.metadata.editableSvg && !isSvgImage(node)) {
                void createEditableImageNode(node);
                return;
            }
            setSvgEditNodeId(node.id);
        },
        [createEditableImageNode, message],
    );
    ...
    const saveSvgNode = useCallback(
        async (node: CanvasNodeData, svg: string) => {
            try {
                        ...
                        item.id === node.id
                            ? {
                                  ...
                                  metadata: {
                                      ...
                                      ...imageMetadata(image),
                                      editableSvg: clean,
                                      freeResize: item.metadata?.freeResize || false,
                                  },
                              }
                            : item,
                    ),
                ...
                setSvgEditNodeId(null);
                message.success("SVG 已保存");
            } catch (error) {
                message.error(error instanceof Error ? error.message : "保存 SVG 失败");
            }
        },
        [message],
    );
    ...
    return (
        <main className="flex h-full min-h-0 overflow-hidden" style={{ background: theme.canvas.background, color: theme.node.text }}>
            <section className="relative min-w-0 flex-1 overflow-hidden">
                ...
                {svgEditNode?.metadata?.content ? <CanvasNodeSvgEditDialog source={svgEditNode.metadata.content} initialSvg={svgEditNode.metadata.editableSvg} open={Boolean(svgEditNode)} onClose={() => setSvgEditNodeId(null)} onConfirm={(svg) => void saveSvgNode(svgEditNode!, svg)} /> : null}
...
function imageExtension(dataUrl: string, mimeType?: string) {
    const extension = dataUrl.match(/^data:image[/]([^;]+)/)?.[1] || mimeType?.match(/image[/]([^;]+)/)?.[1] || dataUrl.match(/image[/]([^;]+)/)?.[1] || "png";
...
function isSvgImage(node: CanvasNodeData) {
...
async function hydrateCanvasImages(nodes: CanvasNodeData[]) {
    return Promise.all(
        nodes.map(async (node) => {
            ...
            if (content.startsWith("data:image/svg+xml")) return { ...node, metadata: { ...node.metadata, editableSvg: node.metadata?.editableSvg || sanitizeEditableSvg(decodeSvgDataUrl(content)), mimeType: "image/svg+xml" } };
